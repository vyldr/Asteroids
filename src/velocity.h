...
public:
   // constructors
   Velocity() :       point(), dx(0.0), dy(0.0)   { point.setWrap(true); };
   Velocity(const Velocity &rhs)                  { *this = rhs;  point.setWrap(true);  };
   Velocity(float x, float y, float dx, float dy) : point(x, y), dx(dx), dy(dy) { point.setWrap(true); };
